...
import com.gabriel.wayairlinesapp.databinding.FragmentFlightsTravelBinding
...
@AndroidEntryPoint
class FlightsTravelFragment : Fragment() {

    private val viewModel: FlightStatusViewModel by viewModels()

    private var _binding: FragmentFlightsTravelBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentFlightsTravelBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    private fun initRecyclerView(flights: List<Flight>) {
        with(binding.rvFlights) {
            ...
            adapter = FlightAdapter(flights) { flightId ->
                val action =
                    FlightsTravelFragmentDirections.actionFlightsTravelFragmentToFlightDetailsFragment(
                        flightId
                    )
