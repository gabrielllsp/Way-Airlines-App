...
import com.gabriel.wayairlinesapp.databinding.FragmentCompletedFlightsBinding
...
@AndroidEntryPoint
class CompletedFlightsFragment : Fragment() {

    private var _binding: FragmentCompletedFlightsBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentCompletedFlightsBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    private fun initRecyclerView(flights: List<Flight>) {
        with(binding.rvFlights) {
            ...
            adapter = FlightAdapter(flights) { flightId ->
                val action =
                    CompletedFlightsFragmentDirections.actionCompletedFlightsFragmentToFlightDetailsFragment(
                        flightId
                    )
